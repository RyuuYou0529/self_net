...
def train(train_set,netD_A,netD_B,netG_A,netG_B,deblur_net,args,device,criterionGAN,criterionL1,SSIM_loss,optimD,optimG,optim_deblur):
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_set):
            ...
            # ACCORD Loss
            accord_B = netG_A(rec_A)
            accord_A = netG_B(rec_B)
            loss_accord_G_A = lambda_accord*criterionL1(fake_B, accord_B) + lambda_ssim*(1-SSIM_loss(fake_B, accord_B))
            loss_accord_G_B = lambda_accord*criterionL1(fake_A, accord_A) + lambda_ssim*(1-SSIM_loss(fake_A, accord_A))
            writer.add_scalar('Loss_accord/loss_accord_G_A', loss_accord_G_A, global_step=total_iters)
            writer.add_scalar('Loss_accord/loss_accord_G_B', loss_accord_G_B, global_step=total_iters)
            ...
            # loss_G = loss_G_A + loss_G_B + loss_feedback + loss_accord_G_A + loss_accord_G_B
            loss_G = loss_G_A + loss_G_B + loss_cycle_A + loss_cycle_B + loss_feedback + loss_accord_G_A + loss_accord_G_B
...
def main():
    ...
    parser.add_argument('--path', default='/home/ryuuyou/Project/self_net/data/care_liver')
    ...
    train_set=pytorch_dataset.create_train_data(os.path.join(args.path, 'train_data/'),args.batch_size,args.min_v,args.max_v)
